...
static enum CancelerResult CancelerTruant(struct BattleCalcValues *cv)
{
    if (GetBattlerAbility(cv->battlerAtk) == ABILITY_TRUANT
        && gBattleMons[cv->battlerAtk].volatiles.truantCounter
        && RandomPercentage(RNG_TRUANT, 70))
    {
        ...
        gBattleStruct->moveResultFlags[cv->battlerDef] |= MOVE_RESULT_MISSED;
        if (!IsBattlerAtMaxHp(cv->battlerAtk) && !gBattleMons[cv->battlerAtk].volatiles.healBlock)
        {
            BattleScriptPushCursorAndCallback(BattleScript_IceBodyHeal);
            gBattleStruct->moveDamage[cv->battlerAtk] = GetNonDynamaxMaxHP(cv->battlerAtk) / 16;
            if (gBattleStruct->moveDamage[cv->battlerAtk] == 0)
                gBattleStruct->moveDamage[cv->battlerAtk] = 1;
            gBattleStruct->moveDamage[cv->battlerAtk] *= -1;
        }
        return CANCELER_RESULT_FAILURE;
    }
...
static bool32 HandleMoveTargetRedirection(struct BattleCalcValues *cv, enum MoveTarget moveTarget)
{
    ...
    bool32 currTargetCantAbsorb = ((ability != ABILITY_LIGHTNING_ROD 
                                    && ability != ABILITY_GEOELECTRIC)
...
static enum CancelerResult CancelerProtean(struct BattleCalcValues *cv)
{
    ...
    {
        if (GetConfig(B_PROTEAN_LIBERO_GAUCHO) >= GEN_9)
            gBattleMons[cv->battlerAtk].volatiles.usedProteanLiberoGauchoPants = TRUE;
