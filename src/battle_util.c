...
static inline uq4_12_t GetAttackerAbilitiesModifier(enum BattlerId battlerAtk, enum BattlerId battlerDef, uq4_12_t typeEffectivenessModifier, bool32 isCrit, enum Ability abilityAtk)
{
    switch (abilityAtk)
    {
...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct BattleContext *ctx)
{
    ...
    case ABILITY_BURN_ENTRY:
        if (IsMoveMakingContact(ctx->battlerAtk, ctx->battlerDef, ABILITY_NONE, ctx->holdEffectAtk, ctx->move))
            return UQ_4_12(0.75);
        break;
    case ABILITY_THICK_FAT:
        if (IsMoveMakingContact(ctx->battlerAtk, ctx->battlerDef, ABILITY_NONE, ctx->holdEffectAtk, ctx->move))
