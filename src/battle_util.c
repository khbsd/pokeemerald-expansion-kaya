...
static bool32 IsNonVolatileStatusBlocked(u32 battlerDef, enum Abilities abilityDef, enum Abilities abilityAffected, const u8 *battleScript, enum FunctionCallOption option);
...
static bool32 ShouldTeraShellDistortTypeMatchups(u32 move, u32 battlerDef, enum Abilities abilityDef)
{
...
{
    enum Abilities ability = GetBattlerAbility(battlerAtk);
...
bool32 HandleMoveTargetRedirection(void)
{
    ...
    enum Abilities ability = GetBattlerAbility(gBattleStruct->moveTarget[gBattlerAttacker]);
...
bool32 IsAbilityAndRecord(u32 battler, enum Abilities battlerAbility, u32 abilityToCheck)
{
...
static enum MoveCanceller CancellerClearFlags(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerStanceChangeOne(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerSkyDrop(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerObedience(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerPowerPoints(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerTruant(struct BattleContext *ctx)
{
    if (GetBattlerAbility(ctx->battlerAtk) == ABILITY_TRUANT
        && gDisableStructs[ctx->battlerAtk].truantCounter
        && RandomPercentage(RNG_TRUANT, 70))
    {
        CancelMultiTurnMoves(ctx->battlerAtk, SKY_DROP_ATTACKCANCELLER_CHECK);
        gHitMarker |= HITMARKER_UNABLE_TO_USE_MOVE;
        ...
        gBattleStruct->moveResultFlags[ctx->battlerDef] |= MOVE_RESULT_MISSED;
        if (!IsBattlerAtMaxHp(ctx->battlerAtk) && !gBattleMons[ctx->battlerAtk].volatiles.healBlock)
        {
            BattleScriptPushCursorAndCallback(BattleScript_IceBodyHeal);
            gBattleStruct->moveDamage[ctx->battlerAtk] = GetNonDynamaxMaxHP(ctx->battlerAtk) / 16;
            if (gBattleStruct->moveDamage[ctx->battlerAtk] == 0)
                gBattleStruct->moveDamage[ctx->battlerAtk] = 1;
            gBattleStruct->moveDamage[ctx->battlerAtk] *= -1;
        }
        return MOVE_STEP_FAILURE;
    }
...
static enum MoveCanceller CancellerFlinch(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerDisabled(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerVolatileBlocked(struct BattleContext *ctx)
{
    if (GetActiveGimmick(ctx->battlerAtk) != GIMMICK_Z_MOVE
...
static enum MoveCanceller CancellerTaunted(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerImprisoned(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerConfused(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerInfatuation(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerBide(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerZMoves(struct BattleContext *ctx)
{
    if (GetActiveGimmick(ctx->battlerAtk) == GIMMICK_Z_MOVE)
    {
...
static enum MoveCanceller CancellerThaw(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerStanceChangeTwo(struct BattleContext *ctx)
{
    if (B_STANCE_CHANGE_FAIL >= GEN_7 && gChosenMove == ctx->currentMove && TryFormChangeBeforeMove())
        return MOVE_STEP_BREAK;
    return MOVE_STEP_SUCCESS;
...
static enum MoveCanceller CancellerPPDeduction(struct BattleContext *ctx)
{
    if (gBattleMons[ctx->battlerAtk].volatiles.multipleTurns
     || gSpecialStatuses[ctx->battlerAtk].dancerUsedMove
     || ctx->currentMove == MOVE_STRUGGLE)
    ...
    if (gBattleStruct->submoveAnnouncement != SUBMOVE_NO_EFFECT)
    {
        ...
        else if (CancellerVolatileBlocked(ctx) == MOVE_STEP_FAILURE) // Check Gravity/Heal Block/Throat Chop for Submove
        {
            gBattleStruct->submoveAnnouncement = SUBMOVE_NO_EFFECT;
            return MOVE_STEP_FAILURE;
        }
...
static enum MoveCanceller CancellerWeatherPrimal(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerMoveFailure(struct BattleContext *ctx)
{
    const u8 *battleScript = NULL;
...
static enum MoveCanceller CancellerPowderStatus(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerExplodingDamp(struct BattleContext *ctx)
{
...
static enum MoveCanceller CancellerMultihitMoves(struct BattleContext *ctx)
{
    if (GetMoveEffect(ctx->currentMove) == EFFECT_MULTI_HIT)
    {
        enum Abilities ability = ctx->ability[ctx->battlerAtk];
...
{
    u32 moveTarget = GetBattlerMoveTargetType(ctx->battlerAtk, ctx->currentMove);
    enum Abilities abilityAtk = ctx->ability[ctx->battlerAtk];
    ...
    if (IsSpreadMove(moveTarget))
    {
        for (u32 battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
        {
            ...
            enum Abilities abilityDef = GetBattlerAbility(battlerDef);
...
static enum MoveCanceller (*const sMoveSuccessOrderCancellers[])(struct BattleContext *ctx) =
{
    [CANCELLER_CLEAR_FLAGS] = CancellerClearFlags,
    ...
    [CANCELLER_MOVE_FAILURE] = CancellerMoveFailure,
    [CANCELLER_POWDER_STATUS] = CancellerPowderStatus,
    [CANCELLER_PRIORITY_BLOCK] = CancellerPriorityBlock,
...
bool32 CanAbilityAbsorbMove(u32 battlerAtk, u32 battlerDef, enum Abilities abilityDef, u32 move, u32 moveType, enum FunctionCallOption option)
{
    ...
    case ABILITY_WIND_RIDER:
        if ((IsWindMove(move) || moveType == TYPE_FIRE) && !(GetBattlerMoveTargetType(battlerAtk, move) & MOVE_TARGET_USER))
        {
            effect = MOVE_ABSORBED_BY_STAT_INCREASE_ABILITY;
            statId = STAT_ATK;
        }
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, enum Abilities ability, u32 special, u32 moveArg)
{
    u32 effect = 0;
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_INTIMIDATE:
            ...
            break;
        case ABILITY_TERRIFIER:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gBattlerAttacker = battler;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                SET_STATCHANGER(STAT_ATK, 1, TRUE);
                BattleScriptPushCursorAndCallback(BattleScript_TerrifierActivates);
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        {
            enum Abilities abilityAtk = GetBattlerAbility(gBattlerAttacker);
        ...
        case ABILITY_LOVESTRUCK:
            if (B_ABILITY_TRIGGER_CHANCE >= GEN_4 ? RandomPercentage(RNG_LOVESTRUCK, 30) : RandomChance(RNG_LOVESTRUCK, 1, 3))
            {
                if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
                && IsBattlerAlive(gBattlerAttacker)
                && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
                ...
                && !CanBattlerAvoidContactEffects(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), GetBattlerHoldEffect(gBattlerAttacker, TRUE), move))
                {
                    if (CanBeParalyzed(gBattlerTarget, gBattlerAttacker, GetBattlerAbility(gBattlerAttacker)))
                    {
                        gEffectBattler = gBattlerAttacker;
                        gBattleScripting.battler = gBattlerTarget;
                        gBattleScripting.moveEffect = MOVE_EFFECT_PARALYSIS;
        ...
        case ABILITY_OWN_TEMPO:
            if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT)
             ...
             && IsBattlerTurnDamaged(gBattlerTarget)
             && IsBattlerAlive(gBattlerTarget)
             && (B_ABILITY_TRIGGER_CHANCE >= GEN_4 ? RandomPercentage(RNG_CONFUSION, 30) : RandomChance(RNG_CONFUSION, 1, 3))
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
             ...
             && IsBattlerTurnDamaged(gBattlerTarget)) // Need to actually hit the target
        {
            if (RandomPercentage(RNG_ENVENOMATE_POISON, 30)
                && CanBePoisoned(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), GetBattlerAbility(gBattlerTarget)))
            {
                gEffectBattler = gBattlerTarget;
                gBattleScripting.battler = gBattlerAttacker;
                gBattleScripting.moveEffect = MOVE_EFFECT_POISON;
...
static inline bool32 CanBreakThroughAbility(u32 battlerAtk, u32 battlerDef, enum Abilities ability, u32 hasAbilityShield, u32 ignoreMoldBreaker)
{
...
enum Abilities GetBattlerAbilityNoAbilityShield(u32 battler)
{
...
enum Abilities GetBattlerAbilityInternal(u32 battler, u32 ignoreMoldBreaker, u32 noAbilityShield)
{
...
u32 IsAbilityOnFieldExcept(u32 battler, enum Abilities ability)
{
    u32 i;
...
u32 IsAbilityPreventingEscape(u32 battler)
{
    ...
    for (u32 battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        enum Abilities ability = GetBattlerAbility(battlerDef);
...
bool32 CanBeSlept(u32 battlerAtk, u32 battlerDef, enum Abilities abilityDef, enum SleepClauseBlock isBlockedBySleepClause)
{
...
bool32 CanBePoisoned(u32 battlerAtk, u32 battlerDef, enum Abilities abilityAtk, enum Abilities abilityDef)
{
    if (CanSetNonVolatileStatus(
            battlerAtk,
            battlerDef,
...
bool32 CanBeBurned(u32 battlerAtk, u32 battlerDef, enum Abilities abilityDef)
{
    if (CanSetNonVolatileStatus(
            battlerAtk,
            battlerDef,
...
bool32 CanSetNonVolatileStatus(u32 battlerAtk, u32 battlerDef, enum Abilities abilityAtk, enum Abilities abilityDef, enum MoveEffect effect, enum FunctionCallOption option)
{
    const u8 *battleScript = NULL;
...
// This is hacky and could potentially be improved because ideally this function wouldn't exist but there is some inconsitency with timings.
static bool32 ShouldWaitWithActivation(u32 battler, enum ItemHoldEffect holdEffect)
{
    ...
    // Restore HP berries can be knocked off
    if (gBattleStruct->battlerState[battler].itemCanBeKnockedOff)
        return TRUE;

    if (IsBattlerTurnDamaged(battler)
     && gMultiHitCounter == 0 // Non multi hit moves don't wait for Berserk / Anger Shell activation
     && IsBattlerAlive(battler)
     && !TestIfSheerForceAffected(battler, gCurrentMove)
     && HadMoreThanHalfHpNowDoesnt(battler))
    {
        switch (GetBattlerAbility(battler))
        {
        case ABILITY_BERSERK:
            return CompareStat(battler, STAT_SPATK, MAX_STAT_STAGE, CMP_LESS_THAN);
        case ABILITY_ANGER_SHELL:
            return TRUE;
        default:
            return FALSE;
...
u32 ItemBattleEffects(enum ItemCaseId caseID, u32 battler)
{
    ...
    case ITEMEFFECT_MOVE_END:
        for (battler = 0; battler < gBattlersCount; battler++)
        {
            ...
            gLastUsedItem = gBattleMons[battler].item;
            effect = ItemEffectMoveEnd(battler, holdEffect);
            if (effect)
            {
                gPotentialItemEffectBattler = gBattleScripting.battler = battler;
                if (effect == ITEM_STATUS_CHANGE)
        ...
        case HOLD_EFFECT_FLINCH:
            {
                ...
                if (IsBattlerTurnDamaged(gBattlerTarget)
                    ...
                    && (ability != ABILITY_STENCH && ability != ABILITY_OMINOUS_LUCK && ability != ABILITY_BLESSED_LUCK))
                {
                    gBattleScripting.moveEffect = MOVE_EFFECT_FLINCH;
                    BattleScriptPushCursor();
                    SetMoveEffect(gBattlerAttacker, gBattlerTarget, FALSE, FALSE);
...
bool32 IsBattlerGroundedInverseCheck(u32 battler, enum Abilities ability, enum ItemHoldEffect holdEffect, enum InverseBattleCheck checkInverse)
{
    ...
    if (holdEffect == HOLD_EFFECT_AIR_BALLOON)
        return FALSE;
    if (ability == ABILITY_LEVITATE || ability == ABILITY_FLUX_FIELD)
        return FALSE;
...
u32 GetBattlerWeight(u32 battler)
{
    ...
    enum Abilities ability = GetBattlerAbility(battler);
    enum ItemHoldEffect holdEffect = GetBattlerHoldEffect(battler, TRUE);
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    case ABILITY_SYNERGETIC:
        ...
        {
            if (IsBattleMovePhysical(move) && (spAtk > atkStat))
            {
                atkStat = spAtk;
                atkStage = gBattleMons[battlerAtk].statStages[STAT_SPATK];
            }
            else if (IsBattleMoveSpecial(move) && (atk > atkStat))
            {
                atkStat = atk;
                atkStage = gBattleMons[battlerAtk].statStages[STAT_ATK];
            }
...
static inline uq4_12_t GetAttackerAbilitiesModifier(u32 battlerAtk, u32 battlerDef, uq4_12_t typeEffectivenessModifier, bool32 isCrit, enum Abilities abilityAtk)
{
    switch (abilityAtk)
    {
    ...
    case ABILITY_SNIPER:
        if (isCrit)
            return UQ_4_12(1.5);
    case ABILITY_TOXIC_ASSASSIN:
        if (isCrit && gBattleMons[battlerDef].status1 & STATUS1_PSN_ANY)
            return UQ_4_12(1.25);
        break;
...
static inline uq4_12_t GetDefenderAbilitiesModifier(struct DamageContext *ctx)
{
    ...
    case ABILITY_BURN_ENTRY:
        if (IsMoveMakingContact(ctx->move, ctx->battlerAtk, ABILITY_NONE, ctx->holdEffectAtk, ctx->move))
            return UQ_4_12(0.75);
        break;
    case ABILITY_THICK_FAT:
        if (IsMoveMakingContact(ctx->move, ctx->battlerAtk, ABILITY_NONE, ctx->holdEffectAtk, ctx->move))
...
static inline uq4_12_t GetOtherModifiers(struct DamageContext *ctx)
{
    ...
    {
        DAMAGE_MULTIPLY_MODIFIER(GetAttackerAbilitiesModifier(ctx->battlerAtk, ctx->battlerDef, ctx->typeEffectivenessModifier, ctx->isCrit, ctx->abilityAtk));
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    enum Abilities defAbility = GetBattlerAbility(ctx->battlerDef);
    ...
    {
        modifier = UQ_4_12(0.0);
        if (defAbility == ABILITY_LEVITATE || defAbility == ABILITY_FLUX_FIELD)
        {
...
{
    uq4_12_t modifier = UQ_4_12(1.0);
    enum Abilities abilityDef = GetMonAbility(mon);
...
bool32 TryBattleFormChange(u32 battler, enum FormChanges method)
{
    ...
    else if (GetBattlerPartyState(battler)->changedSpecies != SPECIES_NONE)
    {
        ...
        if (restoreSpecies)
        {
            enum Abilities abilityForm = gBattleMons[battler].ability;
...
bool8 CanMonParticipateInSkyBattle(struct Pokemon *mon)
{
    ...
    bool8 hasLevitateAbility = (GetSpeciesAbility(species, monAbilityNum) == ABILITY_LEVITATE
                                || GetSpeciesAbility(species, monAbilityNum) == ABILITY_FLUX_FIELD);
...
bool32 IsMoveEffectBlockedByTarget(enum Abilities ability)
{
...
void UpdateStallMons(void)
{
    ...
    if (!IsDoubleBattle() || gMovesInfo[gCurrentMove].target == MOVE_TARGET_SELECTED)
    {
        ...
        enum Abilities abilityAtk = GetBattlerAbility(gBattlerAttacker);
        enum Abilities abilityDef = GetBattlerAbility(gBattlerTarget);
