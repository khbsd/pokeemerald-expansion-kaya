...
static enum MoveCanceller CancellerWeatherPrimal(struct BattleContext *ctx)
{
    ...
    if (HasWeatherEffect() && GetMovePower(ctx->currentMove) > 0)
    {
        ...
        if (moveType == TYPE_FIRE && (gBattleWeather & B_WEATHER_RAIN_PRIMAL) && (B_POWDER_RAIN >= GEN_7 || !TryActivatePowderStatus(ctx->currentMove)))
        {
...
static bool32 IsBattlerGroundedInverseCheck(u32 battler, enum InverseBattleCheck checkInverse, enum IronBallCheck checkIronBall)
{
    ...
    bool32 isFloaterAbility = (gAiLogicData->abilities[battler] == ABILITY_LEVITATE || gAiLogicData->abilities[battler] == ABILITY_FLUX_FIELD);
    bool32 isFloaterAbilityAndRecord = (IsAbilityAndRecord(battler, GetBattlerAbility(battler), ABILITY_LEVITATE) || IsAbilityAndRecord(battler, GetBattlerAbility(battler), ABILITY_FLUX_FIELD));

    if (gAiLogicData->aiCalcInProgress ? isFloaterAbility : isFloaterAbilityAndRecord)
        return FALSE;
...
{
    return B_DESTINY_BOND_FAIL >= GEN_7 && gBattleMons[battler].volatiles.destinyBond;
}
