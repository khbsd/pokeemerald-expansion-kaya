...
static bool8 LoadBattlerSpriteGfx(u32 battler)
{
    if (battler < gBattlersCount)
    {
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
        {
            ...
            DecompressTrainerBackPic(gSaveBlock2Ptr->playerGender + gSaveBlock2Ptr->playerIsKaya, battler);
...
void CreateBattlerSprite(u32 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        {
            SetMultiuseSpriteTemplateToTrainerBack(gSaveBlock2Ptr->playerGender + gSaveBlock2Ptr->playerIsKaya, GetBattlerPosition(B_POSITION_PLAYER_LEFT));
            gBattlerSpriteIds[battler] = CreateSprite(&gMultiuseSpriteTemplate, 0x50,
                                                (8 - gTrainerBacksprites[gSaveBlock2Ptr->playerGender + gSaveBlock2Ptr->playerIsKaya].coordinates.size) * 4 + 80,
