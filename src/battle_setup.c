...
EWRAM_DATA bool32 gChoosingStarter = 0;
...
{
    CreateFemaleMon(&gEnemyParty[0], SPECIES_WEEDLE, 5);
    LockPlayerFieldControls();
    gMain.savedCallback = CB2_ReturnToFieldContinueScriptPlayMapMusic;
    gBattleTypeFlags = BATTLE_TYPE_CATCH_TUTORIAL;
    CreateBattleStartTask(B_TRANSITION_SLICE, 0);
...
enum BattleTransition GetTrainerBattleTransition(void)
{
    ...
    if (DoesTrainerHaveMugshot(trainerId) || B_ALL_TRAINERS_HAVE_MUGSHOT)
...
static void TryUpdateGymLeaderRematchFromWild(void)
{
    if (GetGameStat(GAME_STAT_WILD_BATTLES) % B_WILD_COUNT_REMATCH == 0)
        UpdateGymLeaderRematch();
}

static void TryUpdateGymLeaderRematchFromTrainer(void)
{
    if (GetGameStat(GAME_STAT_TRAINER_BATTLES) % B_TRAINER_COUNT_REMATCH == 0)
        UpdateGymLeaderRematch();
}
