...
u32 ScriptGiveMon(u16 species, u8 level, u16 item)
{
    ...
    enum ShinyMode shinyMode = isShiny ? SHINY_MODE_ALWAYS : SHINY_MODE_RANDOM;
    ...
    return ScriptGiveMonParameterized(0, PARTY_SIZE, species, level, item, ITEM_POKE_BALL, NUM_NATURES, NUM_ABILITY_PERSONALITY, MON_GENDERLESS, evs, ivs, moves, shinyMode, FALSE, NUMBER_OF_MON_TYPES, 0);
}
