 ...
 */
static u32 ScriptGiveMonParameterized(u8 side, u8 slot, u16 species, u8 level, u16 item, enum PokeBall ball, u8 nature, u8 abilityNum, u8 gender, u16 *evs, u16 *ivs, enum Move *moves, enum ShinyMode shinyMode, bool8 gmaxFactor, enum Type teraType, u8 dmaxLevel)
{
    struct Pokemon mon;
    u32 i;
    u16 targetSpecies;
    ...
    if (shinyMode == SHINY_MODE_ALWAYS || (P_FLAG_FORCE_SHINY != 0 && FlagGet(P_FLAG_FORCE_SHINY)))
        ...
        isShiny = FALSE;
    else if ((P_STARTER_SHINY || gSaveBlock2Ptr->playerIsKaya) && gChoosingStarter)
        isShiny = TRUE;
    else
        isShiny = GetMonData(&mon, MON_DATA_IS_SHINY);

    if (gChoosingStarter)
        gChoosingStarter = FALSE;
...
void ScrCmd_createmon(struct ScriptContext *ctx)
{
    ...
    u32 numPerfectIvs = gSpeciesInfo[species].perfectIVCount + P_WILD_PERFECT_IVS;
    ...
    if (numPerfectIvs != 0)
    {
        ...
        for (i = 0; i < nonFixedIvCount && i < numPerfectIvs; i++)
        {
