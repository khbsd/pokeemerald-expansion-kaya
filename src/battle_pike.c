...
{
    enum Abilities ability = GetMonAbility(mon);
    bool8 ret = FALSE;
...
static bool8 CanEncounterWildMon(u8 enemyMonLevel)
{
    ...
    {
        enum Abilities monAbility = GetMonAbility(&gPlayerParty[0]);
        if (monAbility == ABILITY_KEEN_EYE || monAbility == ABILITY_INTIMIDATE || monAbility == ABILITY_TERRIFIER)
        {
