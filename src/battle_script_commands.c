...
static bool32 CanAbilityPreventStatLoss(enum Abilities abilityDef);
...
s32 CalcCritChanceStage(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, enum Abilities abilityAtk, enum Abilities abilityDef, enum ItemHoldEffect holdEffectAtk)
{
    ...
    if (critChance != CRITICAL_HIT_BLOCKED
        && (abilityDef == ABILITY_BATTLE_ARMOR
            ...
            || abilityDef == ABILITY_MINERAL_HIDE))
    {
...
static void SetNonVolatileStatus(u32 effectBattler, enum MoveEffect effect, enum StatusTrigger trigger)
{
    ...
    if (effect == MOVE_EFFECT_POISON && gBattleMons[gBattlerAttacker].ability == ABILITY_MERCILESS)
...
static void Cmd_jumpifability(void)
{
    CMD_ARGS(u8 battler, enum Abilities ability, const u8 *jumpInstr);

    u32 battler;
    bool32 hasAbility = FALSE;
    enum Abilities ability = cmd->ability;
...
static void Cmd_jumpifabilitypresent(void)
{
    CMD_ARGS(enum Abilities ability, const u8 *jumpInstr);

    enum Abilities ability = cmd->ability;
    enum Abilities abilityBattler = IsAbilityOnField(ability);
    if (abilityBattler)
    {
...
static bool32 IsElectricAbilityAffected(u32 battler, enum Abilities ability)
{
    u32 moveType;
    ...
    if (moveType == TYPE_ELECTRIC
     && ((ability != ABILITY_LIGHTNING_ROD && ability != ABILITY_GEOELECTRIC) || B_REDIRECT_ABILITY_IMMUNITY >= GEN_5)
...
static void Cmd_setseeded(void)
{
    ...
    else if (IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_GRASS) && gBattleMons[gBattlerAttacker].ability != ABILITY_BALLISTOSPORE)
    {
        gBattleStruct->moveResultFlags[gBattlerTarget] |= MOVE_RESULT_MISSED;
...
static void Cmd_tryrestorehpberry(void)
{
    CMD_ARGS(u8 battler);
    u32 battler = GetBattlerForBattleScript(cmd->battler);
    if (TryRestoreHPBerries(battler, ITEMEFFECT_TRY_HEALING))
        return;
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
{
    NATIVE_ARGS(u8 battler, enum Abilities ability, const u8 *jumpInstr);
    u32 battler = GetBattlerForBattleScript(cmd->battler);
...
void BS_JumpIfIntimidateAbilityPrevented(void)
{
    ...
    enum Abilities ability = GetBattlerAbility(gBattlerTarget);
