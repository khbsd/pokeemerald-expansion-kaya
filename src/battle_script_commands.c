...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    { ITEM_POKE_BALL,       {  35,   _,   _,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_TINY_MUSHROOM,   {  25,  10,   _,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_POKE_BALL,       {   8,  30,   _,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_GREAT_BALL,      {   8,  10,  30,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_POKE_DOLL,       {   8,  10,   9,  30,   _,   _,   _,   _,   _,   _, } },
    { ITEM_BIG_MUSHROOM,    {   3,  10,   9,   _,   _,   _,   _,   _,   _,   _, } },
    { ITEM_GREAT_BALL,      {   3,  10,   9,   9,  30,   _,   _,   _,   _,   _, } },
    { ITEM_NUGGET,          {   3,   3,   9,   8,   9,  30,   _,   _,   _,   _, } },
    { ITEM_PREMIER_BALL,    {   3,   3,   3,   8,   8,   9,  30,   _,   _,   _, } },
    { ITEM_ULTRA_BALL,      {   3,   3,   3,   4,   8,   9,   8,  30,   _,   _, } },
    { ITEM_ETHER,           {   1,   1,   3,   4,   4,   _,   _,   _,   _,   _, } },
    { ITEM_ULTRA_BALL,      {   _,   3,   3,   4,   4,   9,   8,   8,  30,   _, } },
    { ITEM_MOON_STONE,      {   _,   3,   3,   4,   4,   4,   4,   5,   9,  10, } },
    { ITEM_SUN_STONE,       {   _,   3,   3,   4,   4,   4,   4,   5,   9,  10, } },
    { ITEM_PECHA_BERRY,     {   _,   1,   1,   1,   1,   4,   4,   5,   4,   5, } },
    { ITEM_NUGGET,          {   _,   _,   3,   4,   4,   4,   4,   5,   4,   5, } },
    { ITEM_MASTER_BALL,     {   _,   _,   3,   4,   4,   4,   8,   8,   9,  30, } },
    ...
    { ITEM_BIG_NUGGET,      {   _,   _,   _,   _,   _,   _,   _,   1,   1,   1, } },
};
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gPlayerParty[*expMonId]))
                {
                    ...
                    if (gBattleStruct->battlerExpReward)
                    {
                    ...
                    if ((wasSentOut || holdEffect == HOLD_EFFECT_EXP_SHARE) && gBattleStruct->battlerExpReward)
                    {
...
static void ComputeBallData(u32 wildMonBattler, u32 playerBattler, struct BallData *ball)
{
    ...
    if (GetConfig(CONFIG_BADGE_CATCH_BOOST))
        ball->flatBonus += GetNumOwnedBadges() * 5;
}
...
static u32 ComputeCaptureOdds(u32 wildMonBattler, u32 playerBattler)
{
    ...
    u32 badgeCount = GetNumOwnedBadges();
    ...
    if(GetConfig(CONFIG_BADGE_CATCH_BOOST))
        odds = odds * (100 + (badgeCount * 5)) / 100;
