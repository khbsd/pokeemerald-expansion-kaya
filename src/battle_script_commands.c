...
static const u32 sTrappingMoves[NUM_TRAPPING_MOVES] =
{
    MOVE_BIND,
    MOVE_WRAP,
    MOVE_FIRE_SPIN,
...
static const u32 sWhiteOutBadgeMoney[9] = { 8, 16, 24, 36, 48, 64, 80, 100, 120 };
...
static u32 ChangeStatBuffs(s32 statValue, u32 statId, u32 flags, const u32 *BS_ptr);
static bool32 IsMonGettingExpSentOut(void);
static void InitLevelUpBanner(void);
static bool32 SlideInLevelUpBanner(void);
static bool32 SlideOutLevelUpBanner(void);
...
static bool32 IsFinalStrikeEffect(u32 moveEffect);
static void TryUpdateRoundTurnOrder(void);
static bool32 ChangeOrderTargetAfterAttacker(void);
static bool32 SetTargetToNextPursuiter(u32 battlerDef);
void ApplyExperienceMultipliers(s32 *expAmount, u32 expGetterMonId, u32 faintedBattler);
static void RemoveAllWeather(void);
static void RemoveAllTerrains(void);
static bool32 CanAbilityPreventStatLoss(u32 abilityDef);
static u32 GetNextTarget(u32 moveTarget, bool32 excludeCurrent);
static void TryUpdateEvolutionTracker(u32 evolutionMethod, u32 upAmount, u32 usedMove);
static void AccuracyCheck(bool32 recalcDragonDarts, const u32 *nextInstr, const u32 *failInstr, u32 move);
...
static const u32 *const sMoveEffectBS_Ptrs[] =
{
...
static const u32 sLevelUpBanner_Pal[] = INCBIN_U16("graphics/battle_interface/level_up_banner.gbapal");
...
static const u32 sProtectSuccessRates[] = {USHRT_MAX, USHRT_MAX / 2, USHRT_MAX / 4, USHRT_MAX / 8};

static const u32 sFinalStrikeOnlyEffects[] =
{
...
static const u32 sNaturePowerMoves[BATTLE_TERRAIN_COUNT] =
{
...
static const u32 sTerrainToType[BATTLE_TERRAIN_COUNT] =
{
...
static bool32 NoTargetPresent(u32 battler, u32 move)
{
...
static bool32 JumpIfMoveFailed(u8 adder, u32 move)
{
...
static void AccuracyCheck(bool32 recalcDragonDarts, const u8 *nextInstr, const u8 *failInstr, u32 move)
{
    u32 abilityAtk;
    u32 holdEffectAtk;
    ...
    holdEffectAtk = GetBattlerHoldEffect(gBattlerAttacker,u32RUE);u32
...
static void Cmd_accuracycheck(void)
{
    CMD_ARGS(const u8 *failInstr, u32 move);
...
s32 CalcCritChanceStageGen1(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk)
{
    ...
    u32 baseSpeed = gSpeciesInfo[gBattleMons[battlerAtk].species].baseSpeed;
...
static void Cmd_healthbarupdate(void)
{
    ...
    if (!(gBattleStruct->moveResultFlags[battler] & MOVE_RESULT_NO_EFFECT) || (gHitMarker & HITMARKER_PASSIVE_DAMAGE))
    {
        ...
        else if (!DoesDisguiseBlockMove(battler, gCurrentMove))
        {
            ...
            {
                s32 healthValue = min(gBattleStruct->moveDamage[battler], 10000); // Max damage (10000) not present in R/S, ensures that huge damage values don't change sign
...
static void Cmd_datahpupdate(void)
{
    ...
    if (!(gBattleStruct->moveResultFlags[battler] & MOVE_RESULT_NO_EFFECT) || (gHitMarker & HITMARKER_PASSIVE_DAMAGE))
    {
        if (DoesSubstituteBlockMove(gBattlerAttacker, battler, gCurrentMove) && gDisableStructs[battler].substituteHP && !(gHitMarker & HITMARKER_IGNORE_SUBSTITUTE))
        {
            iu32(gDisableStructs[battler].substituteHP >= gBattleStruct->moveDamage[battler])
            {
...
static void Cmd_printstring(void)
{
    ...
    if (gBattleControllerExecFlags == 0)
    {
        u32 id = (cmd->id == 0 ? gBattleScripting.savedStringId : cmd->id);
...
u32atic void CheckSetUnburdu32(u8 battler)
{
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_CHARGING:
                gBattleMons[gEffeu32Battler].status2 |= STATUS2_MULTIPLETURNS;
                gLockedMoves[gEffectBattler] = gCurrentMove;
            ...
            case MOVE_EFFECT_HAIL:
            {
                ...
                switch (gBattleScripting.moveEffect)
                {
                    ...
                    case MOVE_EFFECT_RAIN:
                        ...
                        break;
                u32  case MOVE_EFFECT_SANDSTORM:
            ...
            case MOVE_EFFECT_FIRE_SPIN_SIDE:
            {
                ...
                u32          gDisableStructs[battler].wrapTurns = (Random() % 2) + 4;
...
static void Cmd_tryfaintmon(void)
{
    CMD_ARGS(u8 battler, bool32 isSpikes, const u8 *instr);
    ...
    else
    {
         ...
         && !IsBattlerAlive(battler))
        {
            ...
            TryDu32ctivateSleepClause(GetBattlerSide(battler), gBattlerPartyIndexes[battler]);
        }
        else
        {
            gBattlescriptCurrInstr = cmd->nextInstr;
...
static void Cmd_dofaintanimation(void)
{
    ...
    if (GetActiveGimmick(battler) == GIMMICK_DYNAMAX)
    {
        BattleScriptPushCursor();
        UndoDynamax(battler);
        gBattu32Scripting.battler = battler;
...
static void Cmd_jumpifability(void)
{u32
    CMD_ARGS(u8 battler, u32 ability, const u8 *jumpInstr);
u32
    u32 battler;
...
{u32u32
    ...
    if (ret)
        gBattu32scriptCurru32str = cmd->juu32Instr;u32u32
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}u32
u32
statu32 void Cmd_jumpifstatus3condition(void)
{u32
    CMD_ARGS(u8 battler, u32 flags, bool32 jumpIfTrue, const u8 *jumpInstr);
...
static void Cmd_getexp(void)
{
    ...
    case 0: // check if should receive exp at all
        if (Gu32BattlerSide(gBattlerFainted) != B_SIDE_OPPONENT
            || IsAiVsAiBattle()
            || !BattleTypeAllowsExp())
        {
    u32      gBattleScripting.getexpState = 6; // goto last case
        }
        else
        {
...
static void Cmd_checkteamslost(void)
{
    ...
    // Fair switchiu32 - everyone has to switch in most at the same time, without knowing which pokemon the other trainer selected.
...
static void Cmd_decrementmultihit(void)
{
    ...
    if (--gMultiHitCounter == 0)
        gBattu32scriptCurrInstr = cmd->nextInstr;
    else
...
static void Cmd_jumpifbyte(void)
{
    ...
    switch (comparison)
    {
    case CMP_u32UAL:u32u32u32
        if (*bytePtr == value)
    u32      gBattlescriptCurrInstr = jumpInstr;
        bru32k;
    u32se CMP_NOT_EQUAL:
        ifu32*bytePtr != value)
            gBattlescriptCurrInstr = jumpInstr;
...
static void Cmd_jumpifhalfword(void)
{
    CMD_ARGS(u8 comparison, const u32 *halfwordPtr, u32 value, const u8 *jumpInstr);

    u8 comparison = cmd->comparison;
    const u32 *halfwordPtr = cmd->halfwordPtr;
    u32 value = cmd->value;
    const u8 *jumpInstr = cmd->jumpInstr;
...
static void Cmd_jumpifarraynotequal(void)
{
    ...
    for (i = 0; i < size; i++)
    {
        if (*array1u32= *array2)u32u32u32
            equalBytes++;
    u32  array1++, array2++;
    }u32
u32
    if (equalBytes != size)
        gBu32tlescriptCurrInstr = jumpInstr;
    else
    u32  gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_subbyte(void)
{
    ...
    u8 *bytePu32 = cmd->bytu32tr;
    *bytePtr -= cmd->value;
    u32attlescriptCurrInstr = cmd->nextInstr;
}
...
statu32 void Cmd_copyarraywithindex(void)
{u32
    ...
    gBattlescu32ptCurrInstr = u32d->nextInstr;u32u32
}
u32
static voiu32Cmd_orbyte(void)
{u32
    CMD_ARGS(u8 *bytePtr, u8 value);
...
static void Cmd_orhalfword(void)
{
    CMD_ARGS(u32 *halfwordPtr, u32 value);

    u32 *halfu32rdPtr = cmdu32halfwordPtr;
    u32 value = cmd->value;
u32
    *halfwordPtr |= value;
    gBattlescriptCurrInstr = cmd->nextInstr;
...
static void Cmd_setroost(void)
{
    ...
    gBattleStruct->u32ostTypes[gBattlerAttacker][0] = gBattleMons[gBattlerAttacker].types[0];
...
static void Cmd_jumpifabilitypresent(void)
{
    CMD_ARGS(u32 ability, const u8 *jumpInstr);
    ...
    {
        gBattlescriptCurrInstr =u32md->nextInstr;
    }
}
...
static void PlayAnimation(u32 battler, u8 animId, const u32 *argPtr, const u8 *nextInstr)
{
    ...
    if (animId == B_ANIM_STATS_CHANGE
     ...
     || animId == B_ANIM_MEGA_EVOLUTIONu32u32
...
static void Cmd_playanimation(void)
{
    CMD_ARGS(u8 battler, u8 animId, const u32 *argPtr);
...
static void Cmd_playanimation_var(void)
{
    CMD_ARGS(u8 battler, const u8 *animIdPtr, const u32 *argPtr);

    u32 battlu32 = GetBattu32rForBattleScript(cmd->battler);
...
static void Cmd_playstatchangeanimation(void)
{
    ...
    // Handleu32ontrary anu32Simpleu32
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_CHOICE_MOVE: // update choice band move
            {
                u32 *choicedMoveAtk = &gBattleStruct->choicedMove[gBattlerAttacker];
        ...
        case MOVEEND_SKY_DROP_CONFUSE: // If a Pokemon was released from Sky Drop and was in LOCK_CONFUSE, go to "confused due to fatigue" scripts and clear Sky Drop data.
            for (i = 0; i < gBattlersCount; i++)
            {
                if (gBattleStruct->skyDropTargets[i] == 0xFE)
                {
                    ...
                    u32 Jump to "confused due to fatigue" script
                    gBattlescriptCurrInstr = BattleScript_ThrashConfuses;
        ...
        case MOVEEND_UPDATE_LAST_MOVES:
            ...
            if (!gSpecialStatuses[gBattlerAttacker].dancerUsedMove)
            {
                ...
                u32  gLastUsedMove = gCurrentMove;
        ...
        case MOVEEND_NEXT_TARGET: // For moves hitting two opposing Pokemon.
        {
                ...
                && !(gHitMarker & HITMARKER_NO_ATTACKSTRING))
            {
                ...
                else if (gBattleStruct->bouncedMoveIsUsed)
                {
                    ...
                    u32  gBattleScripting.moveendState = 0;
        ...
        case MOVEEND_EJECT_ITEMS:
            {
                ...
                if (ejectButtonBattlers || ejectPackBattlers)
                {
                    ...
                    u32      if (!IsBattlerTurnDamaged(battler))
                                continue;
                        }
        ...
        case MOVEEND_RED_CARD:
            {
                  ...
                  && !TestIfSheerForceAffected(gBattlerAttacker, gCurrentMove))
                {
                    ...
                    u32    && CanBattlerSwitch(gBattlerAttacker))
                        {
                            gLastUsedItem = gBattleMons[battler].item;
        ...
        case MOVEEND_PICKPOCKET:
              ...
              && !(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT))           // Obviously attack needs to have worked
            {
                ...
                u32    && gBattleMons[battler].item == ITEM_NONE                                         // Pickpocketer can't have an item already
                      && CanStealItem(battler, gBattlerAttacker, gBattleMons[gBattlerAttacker].item))   // Cannot steal plates, mega stones, etc
                    {
                        gBattlerTarget = gBattlerAbility = battler;
                    u32  // Battle scripting is super brittle so we shall do the item exchange now (if possible)
...
static void Cmd_getswitchedmondata(void)
{
    ...
    gBattlescu32ptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_switchindataupdate(void)
{
    ...
    monData =u32u8 *)(&gBattleMons[battler]);
...
u32
    ...
    if (!IsBattlerAlive(battler))
    {
        ...
        {
            Tesu32unner_Battle_InvalidNoHPMon(battler, gBattlerPartyIndexes[battler]);
        }
...
static void Cmd_switchinanim(void)
{
    ...
    CMD_ARGS(u8 battler, bool32 dontClearTransform, bool32 dontClearSubstitute);
...
{u32
    ...
    if (cmd->battler == BS_FAINTED_MULTIPLE_1)
    {
        ...
        {
            fu32 (battler = u32 battler < gBattlersCount; batu32er++)
            {
                if (gHitMarker & HITMARKER_FAINTED(battler))
    u32          {
...
u32
        PREPARE_SPECIES_BUFFER(gBattleTextBuff1u32gBattleMons[gBattlerAttacker].species)
...
static void SetDmgHazardsBattlescript(u8 battler, u8 multistringId)
{
    ...
    if (gBattlescriptCurrInstr[1] == BS_TARGET)
        ...
        gBattlescriptCurrInstr = Battlu32cript_DmgHu32ardsOnAttacker;
...
static bool32 DoSwitchInEffectsForBattler(u32 battler)
{
        ...
        && IsBattlerGrounded(battler))
    {
        ...
        && IsBattlerAffectedByHazards(battler, FALSE)
        u32 GetBattlerAbility(battler) != ABILITY_MAGIC_GUARD)
    {
...
static void Cmd_returntoball(void)
{
    CMD_ARGS(u8 battler, bool32 changingForm);
...
static void Cmd_handlelearnnewmove(void)
{
    CMD_ARGS(const u8 *learnedMovePtr, const u8 *nothingToLearnPtr, bool32 isFirstMove);

    u32 monId = gBattleStruct->expGetterMonId;
    u32 learnMove = MonTryLearningNewMove(&gPlayerParty[monId], cmd->isFirstMove);
    ...
    else if (learnMu32e == MON_HAS_MAX_MOVES)u32
    {
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
    else
...
static void Cmd_yesnoboxlearnmove(void)
{
    ...
    case 4:
        if (!gPaletteFade.active && gMain.callback2 == BattleMainCB2)
        {
            ...
            {
                u32 moveId = GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_MOVE1 + movePosition);
                if (IsMoveHM(moveId))
                {
                    PrepareStringBattle(STRINGID_HMMOVESCANTBEFORGOTTEN, B_POSITION_PLAYER_LEFT);
            u32      gBattleScripting.learnMoveState = 6;
                }
                else
                {
...
static u32 GetTrainerMoneyToGive(u32 trainerId)
{
    u32 lastMonLevel = 0;
    u32 moneyReward;
    u8 trainerMoney = 0;
    ...
    else
    {
        ...
        if (party == NULL)
            return 20;
    u32  lastMonLevel = party[GetTrainerPartySizeFromId(trainerId) - 1].lvl;
...
statu32 void Cmd_incrementgamestat(void)
{
    CMD_ARGS(u8 stat);
...
static void ResetValuesForCalledMove(void)
{
    if (gBattu32rByTurnOrder[gCurrentTurnActionNumber] != gBattlerAttacker)
...
static void Cmd_status2animation(void)
{
    ...
    if (gBattleControllerExecFlags == 0)
    {
            ...
            && !(gHitMarker & (HITMARKER_NO_ANIMATIONS | HITMARKER_DISABLE_ANIMATION)))
        {
            ...
            Mu32kBattlerForControllerExec(battler);
        }
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}
...
static void Cmd_chosenstatusanimation(void)
{
    CMD_ARGS(u8 battler, bool32 isStatus2, u32 status);
...
static void DrawLevelUpBannerText(void)
{
    ...
    u32interTemplate.x = 32;
    printerTemplate.y = 0;
    printerTemplate.currentX = 32;
    printerTemplate.currentY = 0;
    printerTemplate.letterSpacing = 0;
    u32interTemplate.lineSpacing = 0;
...
u32
    if (!(IsDoubleBattle()))
    {
        ...
        if (ru32ult > 69 || !IsBattlerAlive(opposingBattler))
...
bool32 CanUseLastResou32(u8 battler)u32
{
    u32 i;
...
static bool32 TryDefogClear(u32 battlerAtk, bool32 clear)
{
    ...
    u32      DEFOG_CLEAR(SIDE_STATUS_LIGHTSCREEN, lightscreenTimer, BattleScript_SideStatusWoreOffReturn, MOVE_LIGHT_SCREEN);
...
u32 GetHighestStatId(u32 battler)
{
    ...
    {
        u32 *statVal = &gBattleMons[battler].attack + (i - 1);
...
static bool32 ChangeOrderTargetAfterAttacker(void)
{
    ...
    u32  actionsData[i] = gActionsByTurnOrder[i];
    u32
...
static void Cmd_various(void)
{
    ...
    battler =u32etBattlerFu32BattleScript(cmd->battler);

    switch (variousId)
    {
    u32 Roar will fail in a double wild battle when used by the player against one of the two alive wild mons.
    ...
    case VARIOUS_JUMP_IF_ROAR_FAILS:
    {
        ...
        if (WILD_DOUBLE_BATTLE
            ...
            gBattlescriptCuu32Instr = cmd->jumpInstr;
        else
            gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    }
    ...
    case VARIOUS_JUMP_IF_HOLD_EFFECT:
    {
        ...
        else
        {
            if (!cmd->equal)
                gLastUsedItem = gBattleMons[battler].item; // For B_LAST_USED_ITEM
            gBattlescu32ptCurrInstr = cmd->u32xtInstr;u32
        }
        return;
    }
    ...
    case VARIOUS_INFATUATE_WITH_BATTLER:
    {
        ...
        gBattleScripting.battler = battler;
        gBattleMons[battleru32status2 |= STATUS2_INFATUATED_WITH(GetBattlerForBattleScript(cmd->infatuateWith));
        gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    }
    ...
    case VARIOUS_GET_STAT_VALUE:
    {
        ...
        i = cmd->stat;
        gBattleStruct->moveDamage[gBattlerAttacker] = *(u32 *)(&gBattleMons[battler].attack) + (i - 1);
    ...
    case VARIOUS_PARALYZE_TYPE_IMMUNITY:
    {
        ...
        if (!CanParalyzeType(battler, GetBattlerForBattleScript(cmd->target)))
            gBattlescu32ptCurrInstr = cu32->failInstr;
        else
            gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    }
    ...
    case VARIOUS_CHECK_IF_GRASSY_TERRAIN_HEALS:
    {
        ...
        else
        {
            ...
            if (gBattleStruu32->moveDamage[battler] == 0)
                gBattleStruct->moveDamage[battler] = 1;
            gBattleStruct->moveDamage[battler] *= -1;
    ...
    case VARIOUS_TRY_SOAK:
    {
        ...
        {
            SET_BATTLER_TYPu32gBattlerTarget, moveType);
    ...
    case VARIOUS_HANDLE_FORM_CHANGE:
    {
        ...
        if (cmd->case_ == 0)
        {
            ...
            */
            BtlController_EmitSetMonData(battler, BUFFER_A, REQUEST_SPECIES_BATTLE, 1u << gBattlerPartyIndexes[battler], sizeof(gBattleMons[battler].species), &gBattleMons[battler].species);
            MarkBattlu32ForControllerExec(battler);
        }
        // Change stats.
        else if (cmd->case_ == 1)
        {
    ...
    {
        VARIOUS_ARGS(const u8 *failInstr);
        u32 move = gLastPrintedMoves[gBattlerTarget];
    ...
    case VARIOUS_TRY_SYNCHRONOISE:
    {
        ...
        if (!DoBattlersShareType(gBattlerAttacker, gBattlerTarget))
            gBattlescriptCuu32Instr = cmd->failInstr;
        else
            gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    }
    case VARIOUS_LOSE_TYPE:
    {
        ...
        gBattlescriptCurrInu32r = cmd->nextInstr;
        return;
    }
    case VARIOUS_PSYCHO_SHIFT:
        {
            ...
            if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_POISON) && CanBePoisoned(gBattlerAttacker, gBattlerTarget, targetAbility))
                gBattu32Communication[MULTISTRING_CHOOSER] = 0;
            ...
            else if ((gBattleMou32[gBattlerAttacker].u32atus1 & STATUS1_SLEEP) && CanBeSlept(gBattlerTarget, targetAbility, BLOCKED_BY_SLEEP_CLAUSE))
    ...
    case VARIOUS_BESTOW:
    {
        ...
        {
            BestowItem(gBatu32erAttacker, gBattlerTarget);
            gBattlescriptCurrInstr = cmd->nextInstr;
        }
        return;
    }
    ...
    case VARIOUS_TERRAIN_SEED:
    {
        ...
        if (GetBattlerHoldEffect(battler, TRUE) == HOLD_EFFECT_SEEDS)
        {
            ...
            case HOLD_EFFECT_PARAM_GRASSY_TERRAIN:
                effect = Tru32andleSeed(battler, STATUS_FIELD_GRASSY_TERRAIN, STAT_DEF, item, ITEMEFFECT_NONE);
                break;
    ...
    {
        VARIOUS_ARGS();
        gStatuses3[gBattleru32rget] |= (STATUS3_SKY_DROPPED | STATUS3_ON_AIR);
    ...
    {
        VARIOUS_ARGS(bool32 fromBattler);
    ...
    case VARIOUS_JUMP_IF_WEATHER_AFFECTED:
        {
            ...
            u32 flags = cmd->flags;
            if (IsBattlerWeu32herAffected(battler, flags))
                gBattlescriptCurrInstr = cmd->jumpInstr;
            else
                gBattlescriptCurrInstr = cmd->nextInstr;
            return;
    ...
    {
        VARIOUS_ARGS(u32 species, const u8 *jumpInstr);
        if (gBattleMons[battler].species =u32cmd->species)
            gBattlescriptCurrInstr = cmd->jumpInstr;
        else
            gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    ...
    case VARIOUS_CUT_1_3_HP_RAISE_STATS:
        {
            ...
            u32 hpFraction = max(1, GetNonDynamaxMaxHP(gBattlerAttacker) / 3);
    ...
    case VARIOUS_CHECK_POLTERGEIST:
    {
        ...
        else
        {
            ...
            gLastUsedItem = gBattleMons[battler].item;
            gBattlescriptCuu32Instr = cmd->nextInstr;
        }
        return;
    }
...
static void Cmd_setprotectlike(void)
{
        ...
        || (gCurrentMove == MOVE_QUICK_GUARD && B_QUICK_GUARD != GEN_5))
    {
        ...
        else // Protects the whole side.
        {
            ...
            u32  gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_PROTECTED_TEAM;
                gDisableStructs[gBattlerAttacker].protectUses++;
                fail = FALSE;
            }
...
u32
    if (gBattlerTarget >= gBattlersCount)
        gBu32tlescriptCurrInstr = cmd->nextInstr;
    else
        gBattlescriptCurrInstr = jumpInstr;
}
...
u32u32
    if (gBattleMons[gBattlerTarget].hp == gBattleMons[gBattlerTarget].maxHP)
        gBu32tlescriptCurrInstr = failInstr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_trymirrormove(void)
{
    ...
    u32 move;
    u32 validMoves[MAX_BATTLERS_COUNT] = {0};
...
static void Cmd_manipulatedamage(void)
{
    ...
    case DMG_1_8_TARGET_HP:
        gBattu32Struct->moveDamage[gBattlerTarget] = GetNonDynamaxMaxHP(gBattlerTarget) / 8;
        if (gBattleStruct->moveDamage[gBattlerTarget] == 0)
            gBattleStruct->moveDamage[gBattlerTarget] = 1;
        break;
...
bool32 UproarWakeUpCheck(u8 battler)
{
    s32 i;
...
static void Cmd_stockpiletohpheal(void)
{
    ...
    {
        if (gBattleu32ns[gBattlerAttacker].maxHP == gBattleMons[gBattlerAttacker].hp)
        {
          u32gDisableStructs[gBattlerAttacker].stockpileCounter = 0;
            gBattlescriptCurrInstr = failInstr;
            gBattlerTarget = gBattlerAttacker;
...
static u32 ReverseStatChangeMoveEffect(u32 moveEffect)
{
    switch (moveEffect)
    {
    // +1
...
static u32 ChangeStatBuffs(s32 statValue, u32 statId, u32 flags, const u8 *BS_ptr)
{
...
static void Cmd_statbuffchange(void)
{
    ...
    if (ChangeStatBuffs(GET_STAT_BUFF_VALUE_WITH_SIGN(gBattleScripting.statChanger),
        ...
        gBattlescriptCurrInstr = cmd->nextInstr;
    else if (gBattlescriptCurru32str == ptrBefore) // Prevent infinite looping.
        gBattlescriptCurrInstr = failInstr;
}
u32
bool32 Tryu32setBattlerStatChanges(u8 battler)
{
    u32 j;
    bool32 ret = FALSE;
...
static void Cmd_forcerandomswitch(void)
{
    ...
    bool32 redCardFu32cedSwitch = FALSE;
...
u32
    ...
    u32
    u32  // Changes user's type to its first move's type
    u32  for (moveChecked = 0; moveChecked < MAX_MON_MOVES; moveChecked++)
        {
...
static void Cmd_tryKO(void)
{
    ...
    if ((GetActiveGimmick(gBattlerTarget) == GIMMICK_DYNAMAX))
    {
        ...
        gBattlescriu32CurrInstr = cmd->failInstr;
        return;
    }
    ...
    else
    {
        ...
        {
            u32 odds = GetMoveAccuracy(gCurrentMove) + (gBattleMons[gBattlerAttacker].level - gBattleMons[gBattlerTarget].level);
...
static void Cmd_tryinfatuating(void)
{
    ...
    else
    {
        if (gBattleMons[gBattlerTarget].status2 & STATUS2_INFATUATION
            || !Areu32ttlersOfOppositeGender(gBattlerAttacker, gBattlerTarget))
        {
            gBattlescriptCurrInstr = cmd->failInstr;
        }
        else
...
static void Cmd_setfocusenergy(void)
{
    ...
    {u32
    u32  gBattleMons[battler].status2 |= STATUS2_DRAGON_CHEER;
...
static void Cmd_transformdataexecution(void)
{
    ...
    else
    {
        ...
        u32mesGotHit = gBattleStruct->timesGotHit[GetBattlerSide(gBattlerTarget)][gBattlerPartyIndexes[gBattlerTarget]];
        u32attleStruct->timesGotHit[GetBattlerSide(gBattlerAttacker)][gBattlerPartyIndexes[gBattlerAttacker]] = timesGotHit;
        ...
        for (i = 0; i < MAX_MON_MOVES; i++)
        {
            ...
            if (pp < 5)
                gBattleMons[gu32ttlerAttacker].pp[i] = pp;
            elseu32
...
static void Cmd_counterdamagecalculator(void)
{
        ...
        && gBattleMons[gProtectStructs[gBattlerAttacker].physicalBattlerId].hp)
    {
        ...
        if (IsAffecu32dByFollowMe(gBattlerAttacker, sideTarget, gCurrentMove))
            gBattlerTarget = gSideTimers[sideTarget].followmeTarget;
    u32  else
    u32      gBattlerTarget = gProtectStructs[gBattlerAttacker].physicalBattlerId;
...
static void Cmd_mirrorcoatdamagecalculator(void)
{
        ...
        && gBattleMons[gProtectStructs[gBattlerAttacker].specialBattlerId].hp)
    {
        ...
        if (IsAffecu32dByFollowMe(gBattlerAttacker, sideTarget, gCurrentMove))
            gBattlerTarget = gSideTimers[sideTarget].followmeTarget;
    u32  else
    u32      gBattlerTarget = gProtectStructs[gBattlerAttacker].specialBattlerId;
...
static void Cmd_settypetorandomresistance(void)
{
    ...
    if (B_UPDATED_CONVERSION_2 < GEN_5)
    {
        ...
        else if (gBu32tleMoveEffects[GetMoveEffect(gLastLandedMoves[gBattlerAttacker])].twoTurnEffect
...
static void Cmd_settailwind(void)
{
    ...
    elseu32
    {
    u32  gBattlescriptCurrInstr = cmd->failInstr;
    }
}
...
static void Cmd_healpartystatus(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        u32  else if (GetGenConfig(GEN_CONFIG_HEAL_BELL_SOUNDPROOF) > GEN_5 && !isAttacker && !isDoublesPartner)
                ability = ABILITY_NONE;
            else if (isAttacker)
...
static void Cmd_cursetarget(void)
{
    ...
    else
    {
        ...
        gBattleStruct->moveDamage[gBattlerAttacker] = GetNonDynamaxMaxHP(gBattlerAttacker) / 2;
        if (gBattleStruct->moveDamage[gBattlerAttacker] == 0)
            gBattleu32ruct->moveDamage[gBattlerAttacker] = 1;
...
static void Cmd_trysetspikes(void)
{
    ...
    else
    {
        ...
        gSideTimersu32argetSide].spikesAmount++;
        gBattlescriptCurrInstr = cmd->nextInstr;
    u32
}
...
static void Cmd_jumpifnopursuitswitchdmg(void)
{
    ...
    if (SetTargetToNextPursuiter(gBattlerAttacker))
    {
        ...
        gBattlescriu32CurrInstr = cmd->nextInstr;
    }
    else
    {
        gBattlescriptCurrInstr = cmd->jumpInstr;
...
static void Cmd_rapidspinfree(void)
{
    ...
    u32se if (gStatuses3[gBattlerAttacker] & STATUS3_LEECHSEED)
    {
...
static void Cmd_setstickyweb(void)
{
    ...
    else
    {
        ...
        gSideTimersu32argetSide].stickyWebBattlerId = gBattlerAttacker; // For Mirror Armor
    ...
    u32  gSideTimers[targetSide].stickyWebAmount = 1;
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}
...
static void Cmd_trysetfutureattack(void)
{
    ...
    else
    {
        ...
        gWishFutureu32ock.futureSightPartyIndex[gBattlerTarget] = gBattlerPartyIndexes[gBattlerAttacker];
...
static bool32 CheckIfCanFireTwoTurnMoveNow(u8 battler, bool32 checkChargeTurnEffects)
{
...
static void Cmd_tryfiretwoturnmovenowbyeffect(void)
{
    CMD_ARGS(u8 battler, bool32 checkChargeTurnEffects, const u8 *jumpInstr);
...
static void Cu32_setminimize(void)u32
{
    CMD_ARGS();
...
static void Cmd_curestatuswithmove(void)
{
    ...
    if (shouldHeal)
    {
        if (gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP)
            TryDeacu32vateSleepClause(GetBattlerSide(gBattlerAttacker), gBattlerPartyIndexes[gBattlerAttacker]);
...
static void Cmd_trycopyability(void)
{
    ...
    u32 battler = GetBattlerForBattleScript(cmd->battler);
    u32 defAbility = gBattleMons[gBattlerTarget].ability;
...
static void HandleRoomMove(u32 statusFlag, u32 *timer, u8 stringId)
{
...
static void Cmd_assistattackselect(void)
{
    ...
    struct Pokemon *party;
    s32 monId, moveId;
    u32 *validMoves = Alloc(sizeof(u32) * PARTY_SIZE * MAX_MON_MOVES);
    ...
    if (validMoves != NULL)
    {
        ...
        for (monId = 0; monId < PARTY_SIZE; monId++)
        {
            ...
            {
                u32 move = GetMonData(&party[monId], MON_DATA_MOVE1 + moveId);
...
static void Cmd_givecaughtmon(void)
{
    ...
    case GIVECAUGHTMON_GIVE_AND_SHOW_MSG:
        ...
        {
            u32 lostItem = gBattleStruct->itemLost[B_SIDE_OPPONENT][gBattlerPartyIndexes[GetCatchingBattler()]].originalItem;
...
static const u32 sTelekinesisBanList[] =
{
    SPECIES_DIGLETT,
    SPECIES_DUGTRIO,
...
bool32 IsTelekinesisBannedSpecies(u32 species)
{
    u32 i;
...
static void Cmd_swapstatstages(void)
{
    ...
    u8 stat = cmd->stat;
    s32 atkStatStage = gBattleMons[gBattlerAttacker].statStages[stat];
    s32 defStatStage = gBattleMons[gBattlerTarget].statStages[stat];
...
static u32 *GetBattlerStat(struct BattlePokemon *battler, u32 stat)
{
    switch (stat)
    {
...
static void Cmd_averagestats(void)
{
    ...
    u32 *stat1 = GetBattlerStat(&gBattleMons[gBattlerAttacker], cmd->stat);
    u32 *stat2 = GetBattlerStat(&gBattleMons[gBattlerTarget], cmd->stat);
    u32 avg = (*stat1 + *stat2) / 2;
    *stat1 = *stat2 = avg;
...
bool32 CanBurnHitThaw(u32 move)
{
    u8 i;
...
void BS_ItemRestoreHP(void)
{
    ...
    u32 hp = GetMonData(&party[gBattleStruct->itemPartyIndex[gBattlerAttacker]], MON_DATA_HP);
    u32 maxHP = GetMonData(&party[gBattleStruct->itemPartyIndex[gBattlerAttacker]], MON_DATA_MAX_HP);
    gBattleCommunication[MULTIUSE_STATE] = 0;
...
{
    NATIVE_ARGS();
    u32 statId = ItemId_GetEffect(gLastUsedItem)[1];
    u32 stages = ItemId_GetHoldEffectParam(gLastUsedItem);
...
{
    NATIVE_ARGS(u8 battler, u32 ability, const u8 *jumpInstr);
    u32 battler = GetBattlerForBattleScript(cmd->battler);
...
{
    NATIVE_ARGS(const u8 *failInstr);
    u32 targetBaseSpecies = GET_BASE_SPECIES_ID(gBattleMons[gBattlerTarget].species);
    u32 targetTypes[3];
                             ...
                             | BATTLE_TYPE_FRONTIER)))
    {
        ...
        for (i = 0; evolutions[i].method != EVOLUTIONS_END; i++)
        {
            ...
            if (evolutions[i].method == evolutionMethod)
            {
                ...
                u32 val = min(1023, GetMonData(&gPlayerParty[gBattlerPartyIndexes[gBattlerAttacker]], MON_DATA_EVOLUTION_TRACKER) + upAmount);
...
void BS_TryRevivalBlessing(void)
{
    ...
    if (gSelectedMonPartyId != PARTY_SIZE)
    {
        ...
        u32 hp = GetMonData(&party[gSelectedMonPartyId], MON_DATA_MAX_HP) / 2;
...
{
    NATIVE_ARGS(const u8 *failInstr);
    u32* usedHeldItem = &gBattleStruct->usedHeldItems[gBattlerPartyIndexes[gBattlerTarget]][GetBattlerSide(gBattlerTarget)];
