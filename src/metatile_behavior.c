...
static const u8 sTileBitAttributes[NUM_METATILE_BEHAVIORS] =
{
    ...
    [MB_WATERFALL]                          = TILE_FLAG_UNUSED | TILE_FLAG_SURFABLE | TILE_FLAG_HAS_ENCOUNTERS,
    ...
    [MB_SAND_2]                             = TILE_FLAG_UNUSED | TILE_FLAG_SURFABLE | TILE_FLAG_HAS_ENCOUNTERS,
    [MB_SAND_3]                             = TILE_FLAG_UNUSED | TILE_FLAG_SURFABLE | TILE_FLAG_HAS_ENCOUNTERS,
    ...
    [MB_DEOXYS_FORM_CHANGE_NORMAL]          = TILE_FLAG_UNUSED,
    [MB_DEOXYS_FORM_CHANGE_ATTACK]          = TILE_FLAG_UNUSED,
    [MB_DEOXYS_FORM_CHANGE_DEFENSE]         = TILE_FLAG_UNUSED,
    [MB_DEOXYS_FORM_CHANGE_SPEED]           = TILE_FLAG_UNUSED,
...
{
    if (metatileBehavior == MB_SAND || metatileBehavior == MB_SAND_2 || metatileBehavior == MB_SAND_3
        || MetatileBehavior_IsDeepSand(metatileBehavior))
        return TRUE;
    else
        return FALSE;
}
...
bool8 MetatileBehavior_IsForcedMovementTile(u8 metatileBehavior)
{
    if ((metatileBehavior >= MB_WALK_EAST && metatileBehavior <= MB_TRICK_HOUSE_PUZZLE_8_FLOOR)
     ...
     || (metatileBehavior == MB_WATERFALL && !CanAutoUseFieldMove(MOVE_WATERFALL))
     || metatileBehavior == MB_ICE
...
{
    if ((metatileBehavior == MB_TALL_GRASS) || (metatileBehavior == MB_TALL_GRASS_2) || (metatileBehavior == MB_TALL_GRASS_3) || (metatileBehavior == MB_CYCLING_ROAD_PULL_DOWN_GRASS))
        return TRUE;
    else
        return FALSE;
}
