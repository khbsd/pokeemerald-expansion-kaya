...
// returns 0 if item doesnt exist in pocket
u32 GetBagPocketPosition(enum Pocket pocketId, enum Item itemId)
{
    u32 pocketPos = 0;
    if (pocketId >= POCKETS_COUNT)
        return 0;
    if (CurrentBattlePyramidLocation() || FlagGet(FLAG_STORING_ITEMS_IN_PYRAMID_BAG) == TRUE)
        return GetPyramidBagPocketPosition(itemId);
    // Check for item slots that contain the item
    for (pocketPos = 0; pocketPos < gBagPockets[pocketId].capacity; pocketPos++)
    {
        if (GetBagItemId(pocketId, pocketPos) == itemId)
            return pocketPos;
    }
    return 0;
}
...
// returns 0 if item doesnt exist in pocket
u32 GetPyramidBagPocketPosition(enum Item itemId)
{
    u32 pocketPos = 0;
    u16 *items = gSaveBlock2Ptr->frontier.pyramidBag.itemId[gSaveBlock2Ptr->frontier.lvlMode];

    for (pocketPos = 0; pocketPos < PYRAMID_BAG_ITEMS_COUNT; pocketPos++)
    {
        if (items[pocketPos] == itemId)
            return pocketPos;
    }
...
bool32 IsHealingItem(enum Item itemId)
{
