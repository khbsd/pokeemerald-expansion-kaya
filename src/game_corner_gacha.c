...
u16 GetGachaMon(u32 rarity)
{
    ...
    while (!IsMythicalOrLegendary(species) && gachaRolls > 0)
    {
...
u8 CalculateChanceForCategory(u16 owned, u8 baseChance, u16 wager)
{
    u8 newChance;
    u8 ownedPercentage;
    u8 wagerMultiplier;
...
static void CalculatePullOdds(void)
{
    ...
    sGacha->commonChance = CalculateChanceForCategory(sGacha->ownedCommon, RARITY_COMMON_ODDS, wager);
    sGacha->uncommonChance = CalculateChanceForCategory(sGacha->ownedUncommon, RARITY_UNCOMMON_ODDS, wager);
    sGacha->rareChance = CalculateChanceForCategory(sGacha->ownedRare, RARITY_RARE_ODDS, wager);
    sGacha->ultraRareChance = CalculateChanceForCategory(sGacha->ownedUltraRare, RARITY_ULTRA_RARE_ODDS, wager);

    // Final Odds as a sum of chances

    totalChance = sGacha->commonChance + sGacha->uncommonChance + sGacha->rareChance + sGacha->ultraRareChance;
    if (totalChance <= 100)
...
static void GachaMain(u8 taskId)
{
    ...
    case STATE_POKEBALL_ARRIVE_WAIT:
        if (gSprites[sGacha->bouncingPokeballSpriteId].callback == SpriteCallbackDummy)
        {
            ...
            {
                value: sGacha->GachaId + 2,
                method: sGacha->GachaId + 2,
            };

            CreateMon(&gEnemyParty[0], sGacha->CalculatedSpecies, level, Random32(), otId);
