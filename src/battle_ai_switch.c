...
static bool32 FindMonThatAbsorbsOpponentsMove(enum BattlerId battler)
{
    ...
    if ((IsWindMove(incomingMove) || incomingType == TYPE_FIRE) || (isOpposingBattlerChargingOrInvulnerable && IsWindMove(incomingMove)))
    {
    ...
    for (enum BattlerId battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
    {
        ...
        if (AI_CanMoveBeBlockedByTarget(&ctx))
        {
            if ((moveType == TYPE_WATER && abilityDef == ABILITY_STORM_DRAIN)
             ...
             || (moveType == TYPE_ELECTRIC && abilityDef == ABILITY_GEOELECTRIC))
