...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        {
            u32 moveId = FieldMove_GetMoveId(j);
            if (GetMonData(&mons[slotId], i + MON_DATA_MOVE1) == moveId)
            {
                if (moveId != MOVE_FLY) // If Mon already knows FLY, prevent it from being added to action list
    ...
    if (sPartyMenuInternal->numActions < 5 && (CanTeachMove(&mons[slotId], MOVE_FLY) == CAN_LEARN_MOVE) && (CanAutoUseFieldMove(MOVE_FLY) && OW_FLAG_AUTO_USE_FLY))
...
static void SetPartyMonLearnMoveSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (P_ENABLE_MOVE_RELEARNERS || (P_FLAG_TMHM_MOVES != 0 && FlagGet(P_FLAG_TMHM_MOVES)))
    {
...
u8 GetItemEffectType(u16 item)
{
    ...
    else if (itemEffect[4] & ITEM0_POKEVIAL)
