...
static u32 PpStallReduction(enum Move move, enum BattlerId battlerAtk)
{
    ...
    for (u32 partyIndex = 0; partyIndex < PARTY_SIZE; partyIndex++)
    {
        ...
        if (currentStallValue == 0 || GetMonData(&gPlayerParty[partyIndex], MON_DATA_HP) == 0)
            continue;
        PokemonToBattleMon(&gPlayerParty[partyIndex], &gBattleMons[tempBattleMonIndex], FALSE);
        ...
        case EFFECT_LASER_FOCUS:
            if (gBattleMons[battlerDef].volatiles.laserFocus)
                ADJUST_SCORE(-10);
                    ...
                    || aiData->abilities[battlerDef] == ABILITY_MINERAL_HIDE)
                ADJUST_SCORE(-8);
            break;
        case EFFECT_SKETCH:
            if (aiData->lastUsedMove[battlerDef] == MOVE_NONE)
            ...
            case ABILITY_GEOELECTRIC:
                if (moveType == TYPE_ELECTRIC)
                {
                    if (GetConfig(B_REDIRECT_ABILITY_IMMUNITY) < GEN_5
                        && (atkPartnerAbility == ABILITY_LIGHTNING_ROD
                        || atkPartnerAbility == ABILITY_GEOELECTRIC))
                    {
                        RETURN_SCORE_MINUS(10);
                    }
...
static enum MoveComparisonResult CompareGuaranteeFaintTarget(enum BattlerId battlerAtk, enum BattlerId battlerDef, u16 moveSlot1, u16 moveSlot2, enum Move *moves)
{
    s32 dmg1, dmg2;
...
static s32 AI_HPAware(enum BattlerId battlerAtk, enum BattlerId battlerDef, enum Move move, s32 score)
{
    ...
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        if ((effect == EFFECT_HEAL_PULSE || effect == EFFECT_HIT_ENEMY_HEAL_ALLY)
         ...
         || (moveType == TYPE_WATER && (gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_DRY_SKIN || gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_WATER_ABSORB|| gAiLogicData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_BRINEBLOOD)))
        {
            if (gBattleMons[battlerDef].volatiles.healBlock)
                return 0;
