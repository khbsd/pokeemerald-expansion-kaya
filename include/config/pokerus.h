...
#define P_POKERUS_INFECT_AGAIN             FALSE       // If TRUE, your party can get infected even when it is already infected with Pokerus (doesn't affect spreading, only TRUE in gen 2)
#define P_POKERUS_INFECT_EGG               TRUE       // If TRUE, eggs can receive Pokerus from spread and direction infection (gen 2). If FALSE, eggs can receive Pokerus from spread but not direct infection (gen 3). Behavior is unknown in other gens.
...
#define P_POKERUS_HERD_IMMUNITY            FALSE      // If TRUE, pokemon who have been previously infected by the pokerus in your party reduce the chances of your party getting infected by the Pokerus (because they can be rolled at the target of the infection but are now "immune")
...
#define P_POKERUS_INFECTION_ODDS           3 // Actual probability iP_POKERUS_S_INFECTION_ODDS/65536
#define P_POKERUS_SPREAD_ODDS              21846 // Actual probability is P_POKERUS_SPREAD_ODDS/65536 (21846 should correspond to the vanilla Random() % 3)
