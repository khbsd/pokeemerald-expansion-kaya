...
extern const u32 gMonGenders[3];
...
void CreateFemaleMon(struct Pokemon *mon, enum Species species, u8 level);
...
const enum Move *GetSpeciesEggMoves(enum Species species);
...
void PokemonToBattleMon(struct Pokemon *src, struct BattlePokemon *dst, bool32 resetStats);
void CopyPartyMonToBattleData(enum BattlerId battler, u32 partyIndex, bool32 resetStats);
...
u32 GetLevelUpMovesBySpecies(enum Species species, enum Move *moves);
...
u32 AdjustMonGender(u32 personality, u32 gender, enum Species species);
