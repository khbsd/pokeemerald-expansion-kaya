...
struct RfuGameCompatibilityData
{
    ...
    u32 hasNews:1;
    u32 hasCard:1;
    u32 unknown:1; // Never read
    u32 canLinkNationally:1;
    u32 hasNationalDex:1;
    u32 gameClear:1;
    u32 version:4;
    u32 unused:2;
...
struct RfuManager
{
    ...
    /* 0x00e */ bool32 runParentMain2;
    /* 0x00f */ u8 unused2;
    /* 0x010 */ u32 errorParam0;
    /* 0x012 */ u32 errorParam1;
    ...
    /* 0x05c */ bool32 blockReceived[MAX_RFU_PLAYERS];
    /* 0x061 */ bool32 numBlocksReceived[MAX_RFU_PLAYERS];
    /* 0x066 */ u8 idleTaskId;
    /* 0x067 */ u8 searchTaskId;
    /* 0x068 */ u8 unused3[4];
    /* 0x06c */ struct RfuBlockSend sendBlock;
    /* 0x080 */ struct RfuBlockSend recvBlock[MAX_RFU_PLAYERS];
    /* 0x0e4 */ bool32 readyCloseLink[MAX_RFU_PLAYERS];
    /* 0x0e9 */ bool32 readyExitStandby[MAX_RFU_PLAYERS];
    /* 0x0ee */ vu8 errorState;
    /* 0x0ef */ bool32 isShuttingDown;
    /* 0x0f0 */ u8 linkLossRecoveryState;
    /* 0x0f1 */ u8 status;
    /* 0x0f2 */ u32 packet[RFU_PACKET_SIZE];
    /* 0x0fe */ u32 resendExitStandbyTimer;
    /* 0x100 */ u32 allReadyNum;
    ...
    /* 0xcd9 */ bool32 stopNewConnections;
    /* 0xcda */ u8 parentSendSlot;
    /* 0xcdb */ vbool32 parentFinished;
    /* 0xcdc */ vbool32 parentMain2Failed;
    /* 0xcdd */ u8 unused5;
    ...
    /* 0xce8 */ bool32 playerExchangeActive;
...
bool32 Rfu_SendBlockRequest(u8 type);
...
void RfuSetStatus(u8 status, u32 errorInfo);
...
bool32 HasTrainerLeftPartnersList(u32 trainerId, const u8 *name);
void SendRfuStatusToPartner(u8 status, u32 trainerId, const u8 *name);
u32 WaitSendRfuStatusToPartner(u32 trainerId, const u8 *name);
void RequestDisconnectSlotByTrainerNameAndId(const u8 *name, u32 id);
...
void CreateTask_RfuReconnectWithParent(const u8 *name, u32 trainerId);
...
bool32 PlayerHasMetTrainerBefore(u32 id, u8 *name);
void Rfu_DisconnectPlayerById(u32 playerIdx);
u8 GetLinkPlayerInfoFlags(s32 playerId);
void StopUnionRoomLinkManager(void);
bool32 Rfu_GetCompatiblePlayerData(struct RfuGameData *gameData, u8 *username, u8 idx);
bool32 Rfu_GetWonderDistributorPlayerData(struct RfuGameData *gameData, u8 *username, u8 idx);
...
bool32 RfuRecvQueue_Dequeue(struct RfuRecvQueue *queue, u8 *dest);
bool32 RfuSendQueue_Dequeue(struct RfuSendQueue *queue, u8 *dest);
void RfuBackupQueue_Enqueue(struct RfuBackupQueue *queue, const u8 *data);
bool32 RfuBackupQueue_Dequeue(struct RfuBackupQueue *queue, u8 *src);
