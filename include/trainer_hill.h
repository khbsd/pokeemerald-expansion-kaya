...
struct TrainerHillTrainer
{
    ...
    u32 speechAfter[EASY_CHAT_BATTLE_WORDS_COUNT];
...
struct TrainerHillFloorMap
{
    ...
    u32 collisionData[HILL_FLOOR_WIDTH]; // One bit for each tile in column-major order, so every array entry is one row. 1 = impassable, 0 = passable
...
u8 GetTrainerHillOpponentClass(u32 trainerId);
void GetTrainerHillTrainerName(u8 *dst, u32 trainerId);
u8 GetTrainerHillTrainerFrontSpriteId(u32 trainerId);
void InitTrainerHillBattleStruct(void);
void FreeTrainerHillBattleStruct(void);
void CopyTrainerHillTrainerText(u8 which, u32 trainerId);
...
u32 LocalIdToHillTrainerId(u8 localId);
bool32 GetHillTrainerFlag(u8 objectEventId);
void SetHillTrainerFlag(void);
const u8 *GetTrainerHillTrainerScript(void);
void FillHillTrainerParty(void);
void FillHillTrainersParties(void);
u8 GetTrainerEncounterMusicIdInTrainerHill(u32 trainerId);
