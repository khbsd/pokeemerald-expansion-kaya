...
if (-Not (Test-Path ".\pokeemerald_kaya.gba"))
{
    Write-Host "File 'pokeemerald_kaya.gba' not found."
    Write-Host $help
    exit
}
...
&$objdump -D -bbinary -marmv4t -Mforce-thumb --start-address="$start_str" --stop-address="$end_str" .\pokeemerald_kaya.gba > .\pokeemerald_kaya.dump
Invoke-Expression "$DiffTool .\baserom.dump .\pokeemerald_kaya.dump"
